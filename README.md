...
A **data model library**. It reads, validates, and writes the JSON
interchange format that Sienna tools produce and consume, with full type
information.

**What it is not:** a power-flow solver, an optimization framework, or a
simulation engine. It carries no numerics. If you want to *solve* something,
these models are the input format, not the tool.
...
doc = read_document("tests/fixtures/case14_operations.NATURAL_UNITS.json")
